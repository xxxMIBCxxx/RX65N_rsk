...
void encode_ad_converter_init(void)
{
	...
	count = 100;
	while (count-- > 0);	/* 1us wait (ICLK = 120MHz) */

	S12AD.ADCSR.WORD = 0x1200;

	S12AD.ADANSA0.WORD = 0x0008;		/*Channel 3 select */

	S12AD.ADSTRGR.WORD = 0x0100;		/* MTU0.TGRA Compare match trigger */

	S12AD.ADSSTR3 = 0x0B;				/* 1state(PCLKC=60MHz) :16.7ns * 0x0B(state) = 0.18us */

	S12AD.ADCER.WORD = 0x8000;

	IEN(S12ADC0, S12ADI0) = 0;
	IPR(S12ADC0, S12ADI0) = 1;
	IR(S12ADC0, S12ADI0) = 0;
	IEN(S12ADC0, S12ADI0) = 1;
...
void encode_interval_timer_init(uint8_t sampling_rate)
{
	...
	MTU0.TMDR1.BYTE  = 0x00;		/*	b7     Reserved - set to 0.
